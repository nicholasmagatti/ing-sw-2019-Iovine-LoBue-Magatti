...
public class EndTurnController extends Controller {
    ...
    public void update(Event event) {
        ...
        if(event.getCommand().equals(EventType.END_TURN_DUE_USER_DISCONNECTION))
            endTurn();
    }
    ...
    private void checkInfoFromView(String messageJson) {
        ...
        Player ownerPlayer = convertPlayer(infoRequest.getIdPlayer(), infoRequest.getHostNamePlayer());
    ...
    public void endTurn() {
        ...
        if(getTurnManager().getGameTable().isKillshotTrackFull() ||
            ...
            if(player.getPosition() == null) {
                ...
                String mess = "";
                sendInfo(EventType.MSG_BEFORE_ENEMY_ACTION_OR_RELOAD, mess, getHostNameCreateList().addAllExceptOneHostName(player));

                //inside there is the message for Nick
                msgActionLeft(player);
            }
        }
    }
    ...
    private void checkScore() {
        Player ownerPlayer = getTurnManager().getCurrentPlayer();
